...
import { detectHighFrequencyViewing, detectGeoSkip } from './broker-risk';
...
export async function recordListingView(
  ...
  await logAnalytics('listing_detail_viewed', { userId, listingId });
  // ponytail: fire-and-forget behavioral detection on the viewing user. These
  // read recent views and emit BrokerSignal rows if thresholds are breached.
  // Non-fatal — a detection failure must never block the view from recording.
  // Anonymous viewers (null userId) can't be scored, so skip them.
  if (userId) {
    void detectHighFrequencyViewing(userId).catch(() => null);
    void detectGeoSkip(userId).catch(() => null);
  }
