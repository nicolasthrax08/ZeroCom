...
import { selectAlipayAdapter, selectWechatpayAdapter, type PaymentProviderAdapter } from './adapters';
...
export function adapterFor(provider: 'ALIPAY' | 'WECHATPAY'): PaymentProviderAdapter {
  return provider === 'ALIPAY' ? selectAlipayAdapter() : selectWechatpayAdapter();
}
 ...
 *
 * Uses the atomic compare-and-set `store.markPaymentOrderPaid` so concurrent
 * webhook deliveries cannot double-activate.
 */
export async function markPaidFromVerifiedWebhook(input: {
  ...
  const casResult = await store.markPaymentOrderPaid(input.outTradeNo, input.tradeNo);
  if (!casResult) {
    const current = await store.findPaymentOrder(input.outTradeNo);
    if (current && current.status === 'PAID') return current;
    throw new Error(`Order in terminal state: ${current?.status ?? 'UNKNOWN'}`);
  }
  if (casResult.alreadyPaid) {
    return casResult.order;
  }

  const updated = casResult.order;
  await store.appendPaymentAuditLog({
 ...
 * H-06 FIX: Guarded against production use.
 */
export async function devSimulateWebhookSuccess(outTradeNo: string): Promise<PaymentOrder> {
  if (process.env.NODE_ENV === 'production') {
    throw new Error('BLOCKED: devSimulateWebhookSuccess cannot be called in production');
  }
